                ...
                <table class="table table-striped table-hover">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <th>{{$key += 1}}</th>
                            <td>{{$faq->question}}</td>
                            <td style="width: 1px;">{{$faq->answer}}</td>
                            <td>{{$faq->tags}}</td>
                            <td>
                                <label for="">
                                    <input id="{{$faq->id}}" onchange="changeStatus({{$faq->id}})" data-url="{{ route('admin.content.faq.status', $faq->id) }}" type="checkbox" @if ($faq->status === 1) checked @endif>
                                </label>
                            </td>
                ...
                success: function (response) {
                    if(response.status){
                        if(response.checked){
                            ...
                            successToast('پرسش و پاسخ با موفقیت غیرفعال شد.')
                        }
                    }
                    else{
